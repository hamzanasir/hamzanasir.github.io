...
this.svg = {
  ...
  gas: {
    ...
    init(selector) {
      ...
      d3.xml('static/gas.svg', (xml) => {
        ...
        const pipeList = d3.selectAll('#gas-pipes > g, #gas-pipes > path')[0];
        ...
        for (i = 1; i <= 4; i += 1) {
          this.manualValves[`mv${i.toString()}`] = this.svg.selectAll(`#gas-mv${i.toString()}`);
        }
    
        this.cylinders = {
          source: this.svg.selectAll('#gas-source-cylinder'),
          c1: this.svg.selectAll('#gas-cylinder-1'),
          c2: this.svg.selectAll('#gas-cylinder-2'),
          c3: this.svg.selectAll('#gas-cylinder-3'),
        };
    
        this.$selector.find('#gas-valves > .av, .mv').click(function onValveClick() {
          const valveId = $(this)[0].id.split('-')[1];
          if (valveId.indexOf('mv') > -1) {
            svg.gas.setManualValves({ [valveId]: !self.state[valveId] });
          } else {
        ...
        this.$selector.on('keypress', '.gas-flow-setpoint', digitsOnly);
        this.$selector.on('focusout', '.gas-flow-setpoint', function onFlowFocusOut() {
        ...
        this.$selector.on('keypress', '.gas-gas-instance', function onKeyPress(e) {
        ...
        this.$selector.on('focusout', '.gas-gas-instance', function onGasFocusOut() {
          self.manualOptionFocused = false;
        });
        ...
        for (i = 1; i <= 4; i += 1) {
          svg.gas.setManualValves({ [`mv${i.toString()}`]: (Math.random() > 0.5) ? true : false });
        }
      });
    },
    ...
    setPressure(pressure, label, isManometer = false) {
      ...
      let data;
      let led;
      const $circle = this.$selector.find(`#gas-${label}-circle`);
      const $text = this.$selector.find(`#${label}`);
      const $units = this.$selector.find(`#gas-${label}-units`);
      if (isManometer) {
        data = pressure;
        if (has.call(data, 'connected') && data.connected) {
          ...
          $popover = $(`#gas-${label}-indicator > .popover-hover`).attr('data-content', `<div class="metadata">${metadata}</div>`).data('bs.popover');
          $popover.setContent();
    ...
    setFlowController(label, flow) {
      ...
      if (flow && has.call(flow, 'connected') && flow.connected) {
        ...
        if (!this.manualOptionFocused) {
          metadata = `<p><span class="name">Setpoint</span> <span class="value"><span class="gas-flow-setpoint manual-option" contenteditable="${true}" data-label="${label}">${flow.setpoint.toFixed(1)}</span> sccm</span></p>` +
                `<p><span class="name">Gas</span> <span class="value gas-gas-instance manual-option" contenteditable="${this.valveControlEnabled}" data-label="${label}">${flow.gas ? flow.gas.replace(/(\d)/, '<sub>$1</sub>') : 'N/A'}</span></p>` +
  ...
  reactor: {
    ...
    d3.xml('static/reactor.svg', (xml) => {
        this.$selector.html(xml.documentElement);
        this.svg = d3.select(`${selector} > svg`);
  
        this.svg.attr('viewBox', '0 0 313 490')
          .attr('preserveAspectRatio', 'xMinYMin');
        self.resize();
      });
    },
    ...
    resize() {
      const width = this.$selector.width();
      this.svg.attr('width', width).attr('height', width * (490.0 / 313.0));
    },
  
    setTemperature(temperature) {
      const $circle = this.$selector.find('#temperature-circle');
      const $text = this.$selector.find('#temperature-text');
      if (!mock.reactorState) {
        $circle.css({ fill: colors.lightgray });
        return;
      } else if (temperature > 250) {
        $text.css({ fill: colors.white });
        $circle.css({ fill: colors.blue });
      } else {
        $text.css({ fill: colors.black });
        $circle.css({ fill: colors.white });
      }
      $text.text(temperature.toFixed(1));
    },
  
    setPressure(pressure) {
      const $circle = this.$selector.find('#pressure-circle');
      const $text = this.$selector.find('#pressure-text');
      if (!mock.reactorState) {
        $circle.css({ fill: colors.lightgray });
        return;
      } else if (pressure > 5) {
  ...
  filter: {
    ...
    d3.xml('static/filter.svg', (xml) => {
        this.$selector.html(xml.documentElement);
        this.svg = d3.select(`${selector} > svg`);
  
        this.svg.attr('viewBox', '0 0 481 459')
          .attr('preserveAspectRatio', 'xMinYMin');
        self.resize();
      });
    },
    ...
    resize() {
      const width = this.$selector.width();
      this.svg.attr('width', width).attr('height', width * (459.0 / 481.0));
    },
